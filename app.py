...
def predict_charge(age, sex, bmi, children, smoker, region):
    input_df = pd.DataFrame(
        [[age, sex, bmi, int(children), smoker, region]],
...
app = gr.Interface(
    fn=predict_charge,
    inputs=inputs,
    outputs=gr.Number(label="Predicted Insurance Charge ($)"),
    title="Insurance Charges Predictor",
    description="Fill in the details below to predict your estimated insurance charges."
)
